<div class="main">
	...
	<div class="cat_nav">
		...
		foreach ($cat as $v) {
			if (isset($_GET['cat']) && $_GET['cat'] == $v) {
				echo '<a href="/goods?cat='.$v.'" class="cat active">'.htmlspecialchars($v).'</a>';
			} else {
				echo '<a href="/goods?cat='.$v.'" class="cat">'.htmlspecialchars($v).'</a>';
			}
	...
	?>
		<div class="good">
			<?php
			echo '<a href="/goods/description?id='.(int)$row['id'].'"><img src="/img/default/goods/250x250/'.htmlspecialchars($row['main_img']).'" alt=""></a>';
			?>
			<a href="/goods/description?id=<?php echo (int)$row['id']?>" class="name"><?php echo htmlspecialchars($row['name']);?></a>
