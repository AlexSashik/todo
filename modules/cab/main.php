...
//					$errors['active_err']['text']   = 'Приносим наши извинения, но Вы были забанены администратором сайта.';
	...
	if (isset($_POST['login'], $_POST['email'], $_POST['age'], $_POST['pass'], $_POST['pass_repeat'], $_FILES['file'])) {
		...
		if (!isset($err)) {
			...
			$upload_info = Uploader::upload($_FILES['file'], '/img/default/users/100x100');
		...
		if (!isset($err)) {
			...
			if (isset($photo_name)) {
				...
				if ($user['avatar'] != 'noavatar.png') {
					if (file_exists('./img/default/users/100x100/'.$user['avatar'])) unlink('./img/default/users/100x100/'.$user['avatar']);
				} 
