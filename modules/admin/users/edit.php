...
CORE::$JS[]  = '<script defer src="/js/'.CORE::$STYLE.'/users/edit1.00.js"></script>';
	...
	if (empty($_POST['login'])) {
        ...
        if (!empty($_POST['pass'])) {
            if (mb_strlen($_POST['pass'], 'utf-8') < 3) {
                ...
                $new_pass = "`pass` = '".es(myHash($_POST['pass']))."',";
            }
        }
		...
		if (empty($_POST['reg_date'])) {
			...
			$row['reg_date'] = $_POST['reg_date'];
			if (!is_date($_POST['reg_date'])) {
				$err['reg_date'] = 'Формат ввода: '.date('Y-m-d');
			}
		}
				
		// проверка последней активности (поле может быть пустым)
		$row['last_active_date'] = $_POST['last_active_date'];
		if (!empty($row['last_active_date'])) {
			if (preg_match('#^([\d]{4}-[\d]{2}-[\d]{2})\s([0,1]\d|2[0-3]):[0-5]\d:[0-5]\d$#ui', $row['last_active_date'], $matches)) {
		...
		if (!isset($err['reg_date']) && !isset($err['last_active_date']) && !empty($row['last_active_date'])) {
		...
		if ( !isset($err) ) {
			...
			$upload_info = Uploader::upload($_FILES['picture'], '/img/default/users/100x100');
		...
		if ( !isset($err) ) {
			...
			");
			 q ("
				...
				".$photo_add_string."
				`reg_date`    	   = '".es($_POST['reg_date'])."',
				`last_active_date` = '".es($_POST['last_active_date'])."'
				WHERE `id`   	   = ".(int)$_GET['key1']."
			");
