    ...
    if (isset($_POST['name'], $_POST['year'], $_POST['text'], $_FILES['picture'])) {
    	...
    	if (!isset($err)) {
            ...
            if ($upload_info['error'] == 1) {
                ...
                Uploader::resize(100, 100, $upload_info, 'full', false, '/img/default/books/100x100/'.$upload_info['img_name']);
                Uploader::resize(200, 200, $upload_info, 'full', false, '/img/default/books/200x200/'.$upload_info['img_name']);
                unlink('./img/upload'.$upload_info['img_name']);
                $photo_str = "`img_name` = '" . es($upload_info['img_name']) . "',";
                if ($row['img_name'] != 'nophoto.png') {
                    if (file_exists('./img/default/books/100x100/' . $row['img_name'])) unlink('./img/default/books/100x100/' . $row['img_name']);
                    if (file_exists('./img/default/books/200x200/' . $row['img_name'])) unlink('./img/default/books/200x200/' . $row['img_name']);
                }
            }
