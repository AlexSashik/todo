	...
	if (isset($_POST['name'], $_POST['yob'], $_POST['yod'], $_FILES['picture'])) {
		...
		if ( !isset($err) ) {
			...
			if ($upload_info['error'] == 1) {
                ...
                Uploader::resize(100, 100, $upload_info, 'width', false, '/img/default/authors/100x100/'.$upload_info['img_name']);
                //Uploader::resize(200, 200, $upload_info, 'width', false, '/img/default/authors/200x200/'.$upload_info['img_name']);
                unlink('.'.$upload_info['img_src']);
                if ($row['img_name'] != 'nophoto.png') {
                    if (file_exists('./img/default/authors/100x100/'.$row['img_name'])) {
                        unlink('./img/default/authors/100x100/'.$row['img_name']);
                    }
                    if (file_exists('./img/default/authors/200x200/'.$row['img_name'])) {
                        unlink('./img/default/authors/200x200/'.$row['img_name']);
                    }
                }
