		...
		if (!$is_empty) {
			if ($success_count != 0) {
				...
				foreach ($photos as $k => $v) {
					if ($v['error'] == 0) {
						Uploader::resize(100, 100, $v, 'width', false, '/img/default/goods/100x100/'.$v['img_name']);
						Uploader::resize(250, 250, $v, 'width', false, '/img/default/goods/250x250/'.$v['img_name']);
                        unlink('./img/upload/'.$v['img_name']);
