		...
		if ($res_for_del->num_rows) {
			...
			if (!$row_for_del['is_main']) {
				if ($row_for_del['img_name'] != 'no-photo.jpg') {
					if (file_exists('./img/default/goods/100x100/'.$row_for_del['img_name'])) unlink ('./img/default/goods/100x100/'.$row_for_del['img_name']);
					if (file_exists('./img/default/goods/250x250/'.$row_for_del['img_name'])) unlink ('./img/default/goods/250x250/'.$row_for_del['img_name']);
				}
				q ("
				...
				header ("Location: /admin/goods/edit/".(int)$_GET['key1']."");
				exit;
			}
		}
	}
	...
	if (isset($_POST['name'], $_POST['cat'][0], $_POST['is_in_sight'], $_POST['price'], $_POST['text']) && $flag) {
		...
		if (!isset($err)) {
			...
			foreach ($_FILES as $k => $v) {
				if (is_numeric($k)) {
					$upload_info = Uploader::upload($v, '/img/default/goods/250x250');
					if ($upload_info['error'] == 1) {
						if ($upload_info['err_text'] != 'Вы не загрузили файл!') {
							$err[$k] = $upload_info['err_text'];
						}
					} else {
						Uploader::resize(100, 100, $upload_info, 'width', false, '/img/default/goods/100x100/'.$upload_info['img_name']);
						Uploader::resize(250, 250, $upload_info, 'width');
						while ($row2 = $res2->fetch_assoc()) {
							if ($row2['id'] == $k) {
								if ($row2['img_name'] != 'no-photo.jpg') {
									if (file_exists('./img/default/goods/100x100/'.$row2['img_name'])) unlink ('./img/default/goods/100x100/'.$row2['img_name']);
									if (file_exists('./img/default/goods/250x250/'.$row2['img_name'])) unlink ('./img/default/goods/250x250/'.$row2['img_name']);
								}
								q ("
