<?php
CORE::$CSS[] = '<link href="/css/'.CORE::$STYLE.'/goods/goods_list1.02.css" rel="stylesheet" type="text/css">';
CORE::$JS[]  = '<script defer src="/js/'.CORE::$STYLE.'/goods/all_header_footer1.00.js"></script>';
	...
	if (!($res->num_rows)) {
		...
		while ($row = $res->fetch_assoc()) {
			if ($row['img_name'] != 'no-photo.jpg') {
				if (file_exists('./img/default/goods/100x100/'.$row['img_name'])) unlink ('./img/default/goods/100x100/'.$row['img_name']);
				if (file_exists('./img/default/goods/250x250/'.$row['img_name'])) unlink ('./img/default/goods/250x250/'.$row['img_name']);
			}
		}
		q ("
